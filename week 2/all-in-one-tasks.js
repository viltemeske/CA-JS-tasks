...
// [4, 5, 3, 3, 3, 3]
{
    const flatsRooms = flats.map(function (flat) {
        return flat.rooms.length;
    });
    console.log(flatsRooms)
}
console.groupEnd();

console.group('3. Suformuokite butų plotų masyvą');
{
    const flatsAreas = flats.map(function (flats) {
        const flatArea = flats.rooms.reduce(function (prevSum, rooms) {
...
*/
{
    const newFlats  =  flats.map(function (flat) {
        const flatArea = flat.rooms.reduce(function (prevSum, room) {
            return prevSum + room.area;
        }, 0);
        ...
        return {
            ...
            squares: flatArea,
            squarePrice: flatArea / flat.rooms.length,
        }
    });
    console.table(newFlats);
}
console.groupEnd();
